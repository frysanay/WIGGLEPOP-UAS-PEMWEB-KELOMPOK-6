...
<div class="space-y-6 max-w-3xl">
    ...
    <div class="space-y-2">
        ...
        <h1 class="font-heading text-2xl font-bold text-gray-900 dark:text-white">Ubah Produk</h1>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-3xl p-6 md:p-8 border border-gray-300/20 dark:border-gray-800 shadow-sm transition">
        <form action="{{ route('admin.products.update', $product->slug) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 focus:border-brand-purple rounded-xl text-sm outline-none dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" id="price" name="price" value="{{ old('price', intval($product->price)) }}" required min="0"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" id="weight" name="weight" value="{{ old('weight', $product->weight) }}" min="0"
            ...
            @if(is_array($product->images) && count($product->images) > 0)
                <div class="space-y-2">
                    <span class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Foto Saat Ini:</span>
                    <div class="flex flex-wrap gap-3">
                        @foreach($product->images as $img)
                            <div class="w-16 h-16 rounded-xl overflow-hidden bg-gray-50 border border-gray-200">
                                <img src="{{ asset($img) }}" alt="Product Image" class="w-full h-full object-cover">
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <!-- Upload New Images -->
            <div class="space-y-1.5">
                <label for="images" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Upload Foto Baru (Opsional)</label>
                ...
                <p class="text-xs text-gray-400">Pilih foto jika ingin mengubah/mengganti semua foto saat ini.</p>
                @error('images')
                    <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
